      ...
      >
        ...
        >
          <el-card class="box-card">
            <div slot="header" class="clearflex">
              <span>可访问资源</span>
              <el-button
                style="fload: right; padding: 3px 0; color: #ff0000;"
                icon="el-icon-delete"
                type="text"
                @click="deleteRole(r)"
              ></el-button>
  ...
  methods: {
    ...
    doAddRole() {
      if (this.role.name && this.role.nameZh) {
        this.globalLoading = tree;
        this.postRequest("/system/basic/permisss/role", this.role).then(
          resp => {
            if (resp) {
              this.role.name = "";
              this.role.nameZh = "";
    ...
    doUpdate(rid, index) {
      ...
      let selectedKeys = tree.getCheckedKeys(tree);
